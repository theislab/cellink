...
class EQTLPipeline:
    ...
    def _apply_pv_transforms(self, pv: np.ndarray, no_tested_variants: int) -> dict[str, np.ndarray]:
        ...
        return pv_transformed_resultss
    ...
    def _gwas(
        ...
        if self.mode == "best":
            results = self._best_eqtl(pb_data, target_cell_type, target_chrom, target_gene, gwas, no_cis_snips)
        elif self.mode == "all":
            results = self._all_eqtls(pb_data, target_cell_type, target_chrom, target_gene, gwas, no_cis_snips)
        else:
