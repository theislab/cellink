...
def subset_genomic_region(gdata, chrom, start, end):
    ...
    if type(chrom) is not chrom_column_dtype:
        print(
            f"Attempting to convert chrom value {chrom} from type {type(chrom)} to the same dtype as gdata.var['chrom'] which is {chrom_column_dtype}"
        )
        try:
            chrom = chrom_column_dtype(chrom)
        except (ValueError, TypeError) as e:
            raise ValueError(
                f"Failed to convert chromosome {chrom} to the expected dtype {chrom_column_dtype}. Error: {e}"
            ) from e
...
def subset_gene(gdata, gene_id: str or list, gene_snp_map="variant_annotation_vep"):
    ...
    if gene_snp_map not in gdata.uns:
        raise ValueError(
            "VEP annotations must be added to gdata before subsetting by gene. Please run add_vep_annos_to_gdata() first."
        )
    # Ensure that the 'gene_id' column is present in gdata.var
    if "gene_id" not in gdata.uns[gene_snp_map].reset_index().columns:
        raise ValueError(
            "gdata.uns[gene_snp_map] must contain 'gene_id' column. Have you added VEP annotations to gdata?"
        )
