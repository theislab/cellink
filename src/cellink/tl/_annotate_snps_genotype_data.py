...
def add_vep_annos_to_gdata(
    ...
    shared_vars = list(set(gdata.var.index).intersection(annos[id_col]))
    logger.info(f"Subsetting annotations to variants that are in gdata")
    logger.info(f"{len(gdata.var.index) - len(shared_vars)} with missing annotation")
    annos = annos.set_index(id_col).loc[shared_vars].reset_index()
...
def _create_varm_expanded(annos, gdata, id_col, key_prefix="annotations_"):
    ...
    varm = {}
    for key, var in annos_exp.items():
        #merge back to variant ids so that 
        var_m = gdata.var.index.to_frame().join(var, how = "left").drop(columns = "snp_id")
        varm[key] = var_m
