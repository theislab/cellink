"""
sc-linker Step 2: Gene programs → SNP annotations
==================================================

This module ports the GSSG `code/GeneSet_toS2G/` pipeline to Python.

The original pipeline:
  1. geneset_to_bed_sclinker.R  → bedgraphs (Roadmap ∪ ABC + 100kb)
  2. clean_bed.sh               → bedtools sort + merge to remove overlaps
  3. bedgraph_to_annot.py       → SNP annotation (.annot.gz) from .bim + bedgraph

sc-linker runs TWO strategies per program:
  a) ABC_Road_{tissue}: weighted by enhancer activity (Roadmap ∪ ABC links)
  b) 100kb: gene body ± 100kb window (unweighted, binary-ish)

The E-score is:
  E_score(program, trait) = Enrichment(program) - Enrichment(AllCoding_control)

**S-LDSC call used by sc-linker** (from GSSG code/ldsc/run_ldsc_reg.sh):
  ldsc.py --h2 {trait}.sumstats
          --ref-ld-chr {annotations}/{program}/merged.
          --frqfile-chr 1000G_Phase3_frq/1000G.EUR.QC.
          --w-ld-chr weights.hm3_noMHC.
          --overlap-annot --print-coefficients --print-delete-vals
          --out heritability/{program}_h2/{trait}_merged

  NOTE: --h2 (partitioned heritability), NOT --h2-cts (cell-type-specific).

**LDSC bug note** (ldsc issue #342, PR #341):
  When using --overlap-annot with multiple chromosomes, old pandas versions
  sort columns alphabetically during pd.concat in parse.py, causing an IndexError.
  The sclinker-skg author (yyoshiaki) submitted a fix (PR #341) that is not yet
  merged upstream. If your LDSC container uses unpatched ldsc with pandas ≥ 1.0,
  this will crash. See ``LDSCPatchChecker`` below.

Reference data files (download from Broad LDSCORE server):
  Roadmap links: LDSCORE/Jagadeesh_Dey_sclinker/extras/Roadmap_{tissue}_E.txt.gz
  ABC links:     LDSCORE/Jagadeesh_Dey_sclinker/extras/ABCpaper_NasserFulcoEngreitz2020_{tissue}_AvgHiC.txt.gz
  Gene coords:   LDSCORE/Jagadeesh_Dey_sclinker/extras/gene_annotation.txt (TSS positions for 100kb)
"""
...
# Tissue codes used by sc-linker (Jagadeesh*, Dey* et al. Nat. Genet. 2022).
# These correspond to the ABC strategy tissue labels.
TISSUE_CODES = {
    "BLD":  "Blood",
    "BRN":  "Brain",
    "GI":   "Colon/Intestine",
...
# GCS base URL for sc-linker reference files (public, no auth needed).
# Note: the old alkesgroup.broadinstitute.org FTP mirror returns 404.
_SCLINKER_GCS_BASE = (
...
# ---------------------------------------------------------------------------
# What is actually in the GCS bucket
# (confirmed from directory listing 2024-04):
#
#   extras/
#     AllPredictions.AvgHiC.ABC0.015.minus150.withcolnames.ForABCPaper.txt.gz
#                                            ← all-tissue ABC links (373 MB)
#     RoadmapUABCannot_regions_to_genes.txt.gz
#                                            ← all-tissue Roadmap links (207 MB)
#     Roadmap_map_EID_names.txt              ← EID → tissue name mapping
#     RoadmapLinks/links_E001_*.txt, ...     ← per-EID raw Roadmap link files
#     all_genes/ALL/100kb/, 10kb/, ABC_Road_GI_*/
#                                            ← precomputed annots (outputs, not inputs)
#     ldsc_results_examples/                 ← example LDSC results
#
# Per-tissue files like Roadmap_BLD_E.txt.gz are NOT in the bucket.
# They are produced by the GSSG R script (geneset_to_bed_sclinker.R) which
# reads the two large combined files and filters by tissue.
# cellink reproduces this filtering in Python via load_roadmap_links() /
# load_abc_links() after downloading the combined files.
#
# gene_annotation.txt is also NOT in the bucket.
# It is an LDSC gene coordinate file generated by cellink via pybiomart.
# ---------------------------------------------------------------------------
...
def download_sclinker_enhancer_links(
    ...
    *,
    ...
    - ``RoadmapUABCannot_regions_to_genes.txt.gz`` (207 MB) — all-tissue
      Roadmap enhancer-gene activity scores.
    - ``AllPredictions.AvgHiC.ABC0.015.minus150.withcolnames.ForABCPaper.txt.gz``
      (373 MB) — all-tissue ABC model predictions.
    ...
    :func:`load_abc_links` when you pass the downloaded combined files.
...
def _symlink_annots_into_ld_dir(annot_prefix: str, ld_prefix: str) -> None:
    ...
    stem      = Path(annot_prefix).name   # e.g. "B_naive_L2."
    ...
    **Important**: sc-linker uses ``--h2`` (standard partitioned heritability),
    ...
    for program, strategies in ld_prefixes.items():
        ...
        for strategy_name, ld_prefix in strategies.items():
            ...
            # ── Symlink .annot.gz files into the LD score directory ──────────
            # LDSC --overlap-annot needs .annot.gz at the same prefix as .l2.ldscore.gz
            if annotation_prefixes and program in annotation_prefixes:
                annot_prefix = annotation_prefixes[program].get(strategy_name)
            else:
                # Infer: ldscores/.../B_naive_L2. → annotations/.../B_naive_L2.
                annot_prefix = (
                    ld_prefix
            ...
            for sumstats_file in sumstats_files:
                ...
                # sc-linker ref-ld = baseline + program LD scores
...
def download_sclinker_references(
    ...
    tissue: Optional[str] = None,
    download_roadmap: bool = True,
    download_abc: bool = True,
    download_gene_coords: bool = True,
    download_bims: bool = False,   # use get_1000genomes_plink_files() instead
    download_frq: bool = False,    # use get_1000genomes_frq() instead
    download_weights: bool = False, # use get_1000genomes_ld_weights() instead
    download_hapmap3: bool = False, # use get_1000genomes_hapmap3() instead
    download_baseline: bool = False,
    chromosomes: List[int] = list(range(1, 23)),
) -> Dict[str, Path]:
    """
    ...
    **For 1000G files (PLINK, LD scores, weights, frq, HapMap3), use the
    ...
    """
    if any([download_bims, download_frq, download_weights,
            download_hapmap3, download_baseline]):
        logger.warning(
            "download_bims / download_frq / download_weights / download_hapmap3 / "
            "download_baseline are ignored. Use the cellink resource helpers "
            "get_1000genomes_plink_files(), get_1000genomes_frq(), "
            "get_1000genomes_ld_weights(), get_1000genomes_hapmap3() instead — "
            "they download from Zenodo (https://zenodo.org/records/10515792)."
        )

    # Delegate to the focused helper
...
# ---------------------------------------------------------------------------
# Step 2a: Load enhancer-gene links
# ---------------------------------------------------------------------------
...
def load_roadmap_links(
    ...
    """
    # Try comma-separated first (RoadmapUABCannot file is CSV, not TSV)
    # then fall back to tab-separated for other formats
    df = pd.read_csv(roadmap_file, sep=",", compression="infer")
    if len(df.columns) == 1:
        # Only one column → probably tab-separated
        df = pd.read_csv(roadmap_file, sep="\t", compression="infer")
    ...
    if tissue is not None:
        ...
        # Keyword mapping for matching against tissue name columns
        # Roadmap tissue name keywords — matched case-insensitively against
        # the 'tissuename' column in RoadmapUABCannot_regions_to_genes.txt.gz.
        # Tissue names in that file are full descriptive strings from the
        # Roadmap Epigenomics project, e.g.:
        ...
        # 1. Try a direct tissue-name column (RoadmapUABCannot has 'tissuename')
        tissue_col = next(
        ...
        # 2. Fall back to EID map if provided and tissue_col not found
    ...
    # Safety: if filtering removed all rows, fall back to unfiltered and warn
...
def load_abc_links(
    ...
    The actual file in the GCS bucket is
    ``AllPredictions.AvgHiC.ABC0.015.minus150.withcolnames.ForABCPaper.txt.gz``
    — a single combined file for all tissues. Per-tissue files do not exist.
...
# ---------------------------------------------------------------------------
# Gene coordinate file for the 100 kb window strategy
# ---------------------------------------------------------------------------
# LDSC make_annot.py --gene-coord-file expects a space-separated file:
#   GENE CHR START END
#
# The GENE column is matched by plain string equality against the gene-set
# file passed to make_annot.  It must use the SAME identifiers:
#
#   gene_id_type="ensembl"  →  GENE = ENSG IDs  (e.g. ENSG00000099338)
#       Use this when your AnnData var_names are Ensembl IDs — which is
#       the typical case for sc-linker gene programs.
#
#   gene_id_type="hgnc"     →  GENE = HGNC symbols  (e.g. CD19, FOXP3)
#       Use this when your gene-set files contain gene names.
#
# The file does NOT ship with LDSC.  cellink generates both variants from
# Ensembl BioMart (pybiomart) in a single query and caches them.
# ---------------------------------------------------------------------------
...
def _query_biomart_and_write_gene_coords(data_dir: Path) -> None:
    ...
    def _dedup(frame: pd.DataFrame) -> pd.DataFrame:
        ...
        return (
            ...
            [["GENE", "CHR", "START", "END"]]  # keep column order
        )
    ...
    # Write HGNC variant (only rows with a non-empty gene name)
...
def get_gene_annotation(
    ...
    # 3. Generate from BioMart (writes both variants in one query)
...
def genescores_to_abc_road_bedgraph(
    ...
    *,
    ...
    weighted sum of gene scores linked to that interval:
        ...
        score(interval) = gene_score × link_activity
    ...
    ----------
    genescores
        DataFrame (genes × programs) with probabilistic scores in [0, 1].
    ...
    """
    # ── Auto-detect gene columns if not specified ────────────────────────────
    # Common column name variants seen in the sc-linker reference files:
    ...
    # ── Deduplicate + normalise gene names ───────────────────────────────────
    ...
    # ── Pre-index link tables by gene — O(1) lookup per gene per program ────
    # The naive loop does a full table scan (39M rows) per gene per program.
    # Building a dict once reduces this to a single groupby + dict lookup.
    roadmap_activity_col_r = roadmap_activity_col  # local alias for clarity
    ...
    # ── Per-program bedgraph computation ─────────────────────────────────────
...
def genescores_to_100kb_bedgraph(
    ...
    *,
    ...
    # Build gene coord lookup once — deduplication already done at load time
    ...
    # Deduplicate genescores index — AllCoding concat or BioMart can introduce
    # duplicate gene names.  Keep the row with the highest max score per gene.
    if genescores.index.duplicated().any():
        n_before = len(genescores)
        genescores = genescores.groupby(level=0).max()
        logger.debug(f"Deduplicated genescores index: {n_before} → {len(genescores)} genes")

    # Restrict to genes present in the gene annotation
    ...
    # Use reindex (not .loc) to avoid duplicate-index row expansion
    gs_sub = genescores.reindex(common_genes)

    # Build coordinate arrays once — one entry per unique gene
...
# ---------------------------------------------------------------------------
# Step 2c: Bedgraph → SNP annotation (.annot.gz)
# ---------------------------------------------------------------------------
...
def bedgraph_to_snp_annotation(
    ...
    *,
    ...
    # cellink passes --thin-annot to ldsc, which means the .annot.gz file
    # must contain ONLY the annotation column(s), no CHR/BP/SNP/CM header.
    # Writing those 4 string columns causes ldsc to crash with:
...
def genescores_to_annotations(
    ...
    **link_kwargs,
    ...
    """
    out_dir = Path(out_dir).resolve()  # absolute path — required by LDSCRunner
    out_dir.mkdir(parents=True, exist_ok=True)
    ...
    for program in genescores.columns:
        ...
        for strategy_name, bgs in [
            ...
            for chrom in chromosomes:
                ...
                # Filter bedgraph to this chromosome
                chrom_str = str(chrom)
                bg_chrom = bg_df[
                ...
                if len(bg_chrom) == 0:
                    # Write an all-zero annotation for this chromosome so LDSC doesn't fail
...
def compute_ld_scores_for_sclinker(
    ...
    *,
    ...
    # Build the full list of (program, strategy, chrom) jobs first
    # so we can dispatch them all at once to the thread pool.
    ...
    logger.info(
        ...
        f"({len(annotation_prefixes)} programs × "
        f"{sum(len(s) for s in annotation_prefixes.values()) // max(len(annotation_prefixes),1)} strategies × "
...
# ---------------------------------------------------------------------------
# Step 3: S-LDSC heritability regression (sc-linker mode)
# ---------------------------------------------------------------------------

# ---------------------------------------------------------------------------
# Step 4: Parse results and compute E-score
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PPI gene scores (code/calc_PPI_scores)
# ---------------------------------------------------------------------------

def compute_ppi_gene_scores(
    seed_genes: pd.Series,
    ...
    *,
    ...
    """
    Random Walk with Restart (RWR) on a PPI network to propagate gene scores.

    This ports the R scripts `ppi_RWR.R` and `ppi_string_RWR.R` from
    `code/calc_PPI_scores/`. It propagates seed gene scores through the PPI
    network to identify closely connected genes.

    Parameters
    ----------
    seed_genes
        Series mapping gene name → initial score (typically 0 or gene score).
    ppi_network
        Edge list DataFrame with columns for source gene, target gene,
        and optionally edge weight.
    restart_prob
        Probability of restarting at seed genes at each step.
    n_iter
        Number of RWR iterations.
    normalize
        Normalize the adjacency matrix by node degree.
    source_col, target_col
        Column names for edge endpoints.
    weight_col
        Optional edge weight column. If None, uniform weights are used.

    Returns
    -------
    Series
        Gene → propagated score (sums to 1 if normalized).
    """
    # Build gene universe
    all_genes = sorted(set(
        ppi_network[source_col].tolist() +
        ppi_network[target_col].tolist() +
        seed_genes.index.tolist()
    ))
    n = len(all_genes)
    ...
    # Vectorised adjacency matrix build — no per-edge Python loop
    src_genes = ppi_network[source_col].map(gene2idx)
    tgt_genes = ppi_network[target_col].map(gene2idx)
    valid     = src_genes.notna() & tgt_genes.notna()
    i_arr = src_genes[valid].astype(int).to_numpy()
    j_arr = tgt_genes[valid].astype(int).to_numpy()
    if weight_col and weight_col in ppi_network.columns:
        w_arr = ppi_network.loc[valid, weight_col].astype(float).to_numpy()
    else:
        w_arr = np.ones(valid.sum())
    # Undirected: add both directions
    row_arr  = np.concatenate([i_arr, j_arr])
    col_arr  = np.concatenate([j_arr, i_arr])
    data_arr = np.concatenate([w_arr, w_arr])
    A = sp.csr_matrix((data_arr, (row_arr, col_arr)), shape=(n, n))
    ...
    for _ in range(n_iter):
        p_new = (1 - restart_prob) * A.dot(p) + restart_prob * p0
        if np.max(np.abs(p_new - p)) < 1e-10:
            break
        p = p_new

    return pd.Series(p, index=all_genes)


# ---------------------------------------------------------------------------
# LDSC parse.py bug check + patch  (works for local, Docker and Singularity)
# ---------------------------------------------------------------------------

# Unique string present ONLY in the patched version (not in vanilla ldsc)
_PARSE_PY_PATCH_MARKER = "chr_ld[0].columns"

# Exact one-line fix from ldsc PR #341:
#   x = pd.concat(chr_ld)                      ← buggy (columns reordered by pandas)
#   x = pd.concat(chr_ld)[chr_ld[0].columns]   ← fixed (columns locked to first file)
_PARSE_PY_BUGGY_PATTERNS = [
    ...
    # Fallback without the trailing comment (in case whitespace differs)
    (
        "x = pd.concat(chr_ld)",
...
# ---------------------------------------------------------------------------
# Internal helpers — read / write parse.py via runner
# ---------------------------------------------------------------------------
...
def _get_parse_py_path(runner) -> Optional[str]:
    ...
    """
    # 1. Explicit parse_script from runner (preferred)
    explicit = getattr(runner, "parse_script", None) or runner.config.get("parse_script")
    if explicit:
        return str(explicit)

    # 2. Auto-discover from host PATH (local fallback)
...
def _singularity_patch_overlay(runner, sif: str, parse_path: str, patched_tmp: str) -> bool:
    ...
    # Create overlay image if it does not exist yet
    ...
    # Write the patched parse.py into the overlay
    # We create a tiny shell script that does the copy using --overlay --writable
    result = subprocess.run(
        [
            "singularity", "exec",
    ...
    # Record overlay path so run_command mounts it on every subsequent call
...
def _singularity_patch_sandbox(
    ...
    # Convert SIF → sandbox (only once; skip if sandbox directory already exists)
    if not Path(sandbox_path).exists():
        ...
        if result.returncode != 0:
            # Try with --fakeroot for systems without root
    ...
    # Copy patched parse.py into the sandbox (sandbox is just a directory)
    ...
    if not rebuild:
        # Point the runner at the sandbox directory for all future calls
        ...
        # Rebuild a new SIF from the patched sandbox
        ...
        # Swap: backup original, promote new SIF
...
def _merge_bedgraph_bedtools(bg: pd.DataFrame) -> pd.DataFrame:
    ...
    try:
        # bedtools sort (chromosomes may be numeric or "chrN")
        sort_result = subprocess.run(
            ["bedtools", "sort", "-i", tmp_in],
            capture_output=True, text=True, check=True,
        )
        # bedtools merge with sum of scores
...
def _annotate_with_bedtools(bg: pd.DataFrame, bim: pd.DataFrame) -> np.ndarray:
    ...
    # Write BIM as BED (SNPs are 1-bp intervals)
    bim_bed = bim.copy()
    ...
    # Write BIM as BED using pandas — no per-row Python loop
...
def _parse_ldsc_log(log_file: Path) -> Optional[dict]:
    ...
    # .results file lives next to the .log file
