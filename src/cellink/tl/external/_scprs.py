...
if TORCH_AVAILABLE:
    class _AggNet(MessagePassing):
        ...
        def __init__(self, **kwargs):
            ...
            self.parameter1 = nn.Parameter(
                torch.FloatTensor(abs(np.random.rand(1)).reshape(1, 1))
            )
            self.parameter2 = nn.Parameter(
                torch.FloatTensor(abs(np.random.rand(1)).reshape(1, 1))
            )
        ...
        def forward(self, x, edge_index, edge_weight):
            out = self.propagate(
                edge_index=edge_index, edge_weight=edge_weight, x=x
            )
            out += self.bias
    ...
    class _scPRSModel(nn.Module):
        ...
        def __init__(self, dim_in: int, n_cell: int, n_gcn: int):
            ...
            self.parameter = nn.Parameter(
                torch.FloatTensor(
                    abs(np.random.rand(dim_in).reshape([dim_in, 1]))
                )
            )
            self.bias1 = nn.Parameter(
                torch.FloatTensor(np.zeros(1).reshape(1, 1))
            )
        ...
        def forward(self, x, edge_index, edge_weight):
            ...
            x = (
                x @ abs(self.parameter) / x.shape[-1] + self.bias1
...
def _normalise_gwas_df(gwas_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if not has_snp:
        if "CHR" in df.columns and "BP" in df.columns:
            logger.info(
                "No SNP/rsid column — synthesising variant IDs as 'CHR:BP'."
            )
            df["SNP"] = (
...
def _annotate_gwas_with_bim_ids(
    ...
    """
    Harmonise SNP IDs between GWAS summary stats and a PLINK bfile so that
    PLINK ``--clump`` and ``--score`` can find variants by ID.
    ...
    bim["_key"] = (
        bim["CHR"].astype(str).str.replace("chr", "", regex=False)
        + ":"
        + bim["BP"].astype(int).astype(str)
    )
...
def _build_peak_snp_map(
    ...
    try:
        ...
        for chrom, peak_grp in var.groupby("_chrom"):
            ...
            for snp_local, pk_local in zip(snp_local_arr, peak_local_arr):
        ...
        for chrom, peak_grp in var.groupby("_chrom"):
            ...
            snp_ids  = gwas_chrom[snp_col].values
            for bp, snp_id in zip(bps, snp_ids):
...
def _read_prs_score_files(
    ...
    """
    Adaptation of ``step1_process_prs_data`` from
    ``util/generate_data.py`` (https://github.com/szhang1112/scPRS/blob/main/util/generate_data.py).
    ...
    if not res:
        raise RuntimeError(
            f"No PLINK score files found under {prs_dir}. "
            "Make sure the SLURM array job completed successfully."
        )

    res = pd.concat(res, ignore_index=True)

    res["cell"] = res["filename"].apply(lambda x: x.split(".")[0])
    res["type2"] = res["filename"].apply(
    ...
    for row in res.to_dict("records"):
        ...
        ci = d_cell.get(row["cell"])
            ...
            d_sim_cutoff[row["sim_cutoff"]] * len(d_type2)
            + d_type2[row["type2"]]
        )
...
def _build_knn_graph(
    ...
    """
    Adaptation of ``step2_process_atac_data`` from
    ``util/generate_data.py`` (https://github.com/szhang1112/scPRS/blob/main/util/generate_data.py).
...
def get_plink_commands_per_cell(
    ...
    for ci in tqdm(range(adata.n_obs)):
        ...
        gwas_df[gwas_df[snp_col].isin(set(peak_snps))].to_csv(
            cell_gwas_file, sep="\t", index=False
        )
...
def write_slurm_array_job(
    ...
    """
    Prepare per-cell GWAS files and write a SLURM array job for the PLINK C+T
    step.
    ...
    After all array tasks finish, load the results with::
        ...
        pkl = prepare_scprs_data(
            adata=adata, labels=labels, gwas_file=gwas_file,
            prefix="my_prefix", prs_dir="{prs_dir}",
        )
    ...
    logger.info(
        f"Pre-generating per-cell GWAS files in {prs_dir_abs} "
        f"({adata.n_obs} cells) ..."
    )
...
def prepare_scprs_data(
    ...
    for col in ["chrom", "start", "end"]:
        if col not in adata.var.columns:
            raise ValueError(
                f"adata.var must contain '{col}'. "
                "Please annotate peaks with chromosome, start, end coordinates."
            )
    ...
    y = np.array(
        [labels.loc[sid] if sid in labels.index else 0
         for sid in sorted(d_sample, key=d_sample.get)],
        dtype=np.float32,
    )
...
def run_scprs(
    ...
    if not TORCH_AVAILABLE:
        raise ImportError(
            "PyTorch and torch_geometric are required for run_scprs. "
    ...
    for random_state in tqdm(range(n_repeats), desc="scPRS repeats"):
        ...
        for epoch in range(n_epoch):
            ...
            y_id_pos = y_id[y_id[0] == 1].sample(
                n=n_neg, replace=True, random_state=epoch
            )
            select_idn = np.array(
                list(y_id_pos["index"]) + list(y_id[y_id[0] == 0]["index"])
            )
            ...
            for xx, yy in dataloader:
                ...
                loss2 = (
                    abs(model.pred.weight).mean() * l1
                    + (model.pred.weight ** 2).mean() * l2
                )
                loss3 = (
                    model.pred.weight
                    @ L_tensor
...
def get_disease_relevant_cells(
    ...
    ref_per_repeat = np.array(
        [np.percentile(weights[i], top_percentile) for i in range(weights.shape[0])]
    )
