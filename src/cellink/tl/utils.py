...
def _add_dummy_cols(annos, col_to_dummy):
    ...
    dum = pd.get_dummies(
        annos_with_dummy[col_to_dummy], prefix=col_to_dummy, prefix_sep="_", dtype=int
    )
    annos_with_dummy = pd.concat(
        [annos_with_dummy.drop(columns=col_to_dummy), dum], axis=1
    )
    return annos_with_dummy


def _flatten_single_value(lst):
    # If the list has only one unique value, return that value
    if len(set(lst)) == 1:
        return lst[0]
    # Otherwise, return the list as is
    return lst
    logger.info(f"New columns: {dum.columns}")
    return annos_with_dummy
