...
def _sha256sum(filename: Union[str, Path]) -> str:
    """
...
def _download_file(url: str, dest: Path, checksum: Optional[str] = None) -> None:
    """
...
def _run(cmd: str, cwd: Optional[Union[str, Path]] = None) -> subprocess.CompletedProcess:
    """
...
def _to_dataframe(data: Optional[list[dict]]) -> pd.DataFrame:
    """
...
def _cache_df(
    data_home: Path, filename: str, refresh: bool, fetcher: Callable[[], pd.DataFrame]
) -> pd.DataFrame:
    """
