...
DD_FILE = "/Users/selman/cellink_data/dummy_onek1k/dummy_onek1k.dd.h5"


# %%

if os.path.exists("dd_C.zarr"):
    # remove recursively
    shutil.rmtree("dd_C.zarr")

if os.path.exists("dd_G.zarr"):
    # remove recursively
    shutil.rmtree("dd_G.zarr")
...
with h5py.File(DD_FILE, "r") as f:
    DatasetCollection("dd_C.zarr").add_adatas(
        [f["C"]],
        shuffle=True,
        load_adata=load_only_X_lazy,
        # set the params for sharding and shuffling with realistic datasets
    )
    DatasetCollection("dd_G.zarr").add_adatas(
        [f["G"]],
        shuffle=True,
        load_adata=load_only_X_lazy,
        # set the params for sharding and shuffling with realistic datasets
...
def load_obs_from_random_sampler(sampler, obs):
    ...
    for lr in sampler.sample(len(obs)):
        indices = np.concatenate([np.arange(x.start,x.stop) for x in lr["chunks"]])
...
def train_yields(n_epochs):
    ...
    for epoch in range(n_epochs):
        flattened_donors = []
        n_donors_per_batch = []
        for batch_donors in load_obs_from_random_sampler(random_sampler1, obs):
