...
def profile_train_yields(n_epochs, batch_size, chunk_size, preload_nchunks, preload_ndonors, max_batches=20):
    ...
    print(f"Total time spent fetching cells (loader_C): {total_C_time:.4f}s (avg: {total_C_time/batch_count:.4f}s per batch)")
    print(f"Total time spent fetching donors (loader_G): {total_G_time:.4f}s (avg: {total_G_time/batch_count:.4f}s per batch)")
