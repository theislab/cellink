...
if not os.path.exists(DD_CACHE_PATH_ZARR):
    raise FileNotFoundError(
        f"{DD_CACHE_PATH_ZARR} not found. Run build_onek1k_dd_cache.py first to build it."
    )
...
trainer = train_livi_annbatch(
    ...
    cis_snps=N_CIS_SNPS, target_genes=N_TARGET_GENES, cis_genes_per_snp=CIS_GENES_PER_SNP,
    ...
    limit_train_batches=BENCH_BATCHES, enable_progress_bar=True, log_every_n_steps=10,
    callbacks=[ThroughputCallback()],
)
print("\n==== ANNBATCH cis (paper config, full genes) ====")
print(f"epochs: {trainer.current_epoch}  batches/epoch cap: {BENCH_BATCHES}  total wall: {time.perf_counter() - t0:.1f}s")
